...
checkToken = () => {
    ...
    if (tokenInputValue != tokenAlreadyStored && tokenInputValue != null || tokenInputValue === tokenAlreadyStored ) {
    //throw it in
        localStorage.setItem('Authorization', JSON.stringify(tokenInputValue));
        //this will eventually hide/show the input field or whetever we use to show an authenticated user
        $("#token_field_submit").click(e => {
            $("#auth_true_show'").toggleClass('is-active')
        })
        console.log('token that went in: ', tokenInputValue);
    } 
